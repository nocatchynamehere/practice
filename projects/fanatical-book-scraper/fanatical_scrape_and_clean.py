...
for slugs, sale_type in zip(book_slugs, sale_types):  
    ...
    slug_long_list = [make_pretty_slugs(slugs)] * len(books)
    cleaned_authors = [string_authors(a) for a in authors]
    scraped_bundles[0].extend(slug_long_list)
    scraped_bundles[1].extend(books)
    scraped_bundles[2].extend(cleaned_authors)
